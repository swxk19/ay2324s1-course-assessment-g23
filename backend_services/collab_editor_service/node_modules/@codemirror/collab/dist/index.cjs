...
function receiveUpdates(state$1, updates) {
    ...
    if (unconfirmed.length) {
        ...
        if (effects.length) {
            let composed = unconfirmed.reduce((ch, u) => ch.compose(u.changes), state.ChangeSet.empty(unconfirmed[0].changes.length));
            effects = state.StateEffect.mapEffects(effects, composed);
        }
    }
    if (!changes)
...
function rebaseUpdates(updates, over) {
    ...
    return !changes ? updates : updates.map(update => {
        ...
        return {
            ...
            effects: update.effects && state.StateEffect.mapEffects(update.effects, changes),
...
exports.rebaseUpdates = rebaseUpdates;
